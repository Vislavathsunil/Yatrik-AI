...
Ensure the itinerary has exactly ${durationDays} days, with practical, structured recommendations suited to ${numTravelers} travelers seeking a ${travelStyle} trip on a ${budget} budget. Let the attractions, activities, and food recommendations be as contextual and authentic as possible. Keep descriptions concise to ensure fast generation.`;
    ...
    // Attempt to parse JSON to ensure it is valid
    const parsedItinerary = JSON.parse(resultText);
    return res.json(parsedItinerary);
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const systemInstruction = `You are Yatrik AI, an intelligent, friendly travel companion specializing in travel within India.
Your mission is to help users design the perfect custom trip, suggest offbeat/hidden gems, recommend authentic regional cuisines, and help refine their travel plans.

CRITICAL RULES:
1. Always keep the conversation focused on Indian destinations and travel planning. If the user asks about unrelated topics, politely redirect them back to planning their Incredible India escape.
2. If the user wants to plan a trip, summarize the perfect destination suggestion and invite them to click 'AI Plan this Escape' or load it in the planner.`;
